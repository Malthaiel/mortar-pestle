...
//!
//! tokio (multi-thread) `UnixListener` at `$XDG_RUNTIME_DIR/iskariel/stt.sock`.
...
/// `$XDG_RUNTIME_DIR/iskariel/stt.sock` (falls back to `/tmp` if the env var is
/// unset, matching tokio conventions for a session-scoped socket). The
/// `iskariel` parent dir is SHARED across sidecars; only the filename differs
...
/// `\\.\pipe\iskariel-stt` — the Windows named-pipe analogue of the Unix socket
    ...
    match UnixListener::bind(path) {
        ...
        Err(e) if e.kind() == io::ErrorKind::AddrInUse => {
            if probe_live_daemon(path).await {
                log::info!("iskariel-stt daemon already running at {}", path.display());
                Ok(None)
            } else {
...
pub async fn serve(ctx: ControlContext) -> io::Result<()> {
    ...
    let listener = match bind_or_probe(&path).await? {
        ...
        None => {
            ...
            println!("iskariel-stt: daemon already running ({})", path.display());
            std::process::exit(0);
        }
    };
...
/// Windows control server over a named pipe — the `\\.\pipe\iskariel-stt` analogue of
...
pub async fn serve(ctx: ControlContext) -> io::Result<()> {
    let mut server = match ServerOptions::new().first_pipe_instance(true).create(PIPE_NAME) {
        ...
        Err(e) if e.raw_os_error() == Some(5) || e.kind() == io::ErrorKind::PermissionDenied => {
            println!("iskariel-stt: daemon already running ({PIPE_NAME})");
            std::process::exit(0);
        }
        Err(e) => return Err(e),
    };
