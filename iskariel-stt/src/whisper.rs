...
//! iskariel-capture daemon, STT events are self-contained payloads (no engine-state
...
pub fn spawn_worker(
    ...
    std::thread::Builder::new()
        .name("iskariel-stt-whisper".into())
        .spawn(move || run_worker(cmd_rx, events, cancel))
        .expect("spawn iskariel-stt whisper worker thread")
}
