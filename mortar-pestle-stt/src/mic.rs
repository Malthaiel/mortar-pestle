...
impl MicCapture {
    ...
    pub fn open(frames_tx: Sender<AudioChunk>) -> Result<MicHandle, MicError> {
        ...
        let thread = std::thread::Builder::new()
            .name("mortar-pestle-stt-mic".into())
...
fn build_capture_stream(
    ...
    let err_fn = |e: cpal::StreamError| eprintln!("mortar-pestle-stt: cpal stream error: {e}");
    ...
    match format.sample_format {
        ...
        other => {
            eprintln!("mortar-pestle-stt: unsupported sample format: {other}");
