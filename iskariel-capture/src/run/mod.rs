...
//!
//! `iskariel-capture run --dump-es <path.h264> --duration <secs>` captures the
...
//! is judged on. `iskariel-capture decode-proof <es> <mp4>` shells ffmpeg to remux
...
/// Entry point for `iskariel-capture run`.
#[cfg(target_os = "linux")]
pub fn run(args: &[String]) -> ExitCode {
    ...
    let Some(es_path) = es_path else {
        eprintln!("usage: iskariel-capture run --dump-es <path.h264> [--duration <secs>]");
    ...
    match result {
        Ok(()) => {
            ...
            println!("  next: iskariel-capture decode-proof {es_path} <out.mp4>  (eyeball color/tearing = B2/B3)");
            ExitCode::SUCCESS
        }
        Err(e) => {
...
fn build_report(r: &Recorder, elapsed: Duration, res: Option<&ResourceSamples>) -> String {
    ...
    s.push_str("=== iskariel-capture Stage E proof — budget stats ===\n");
...
/// Entry point for `iskariel-capture decode-proof <es> <mp4>` — remux the raw
...
pub fn decode_proof(args: &[String]) -> ExitCode {
    let (Some(es), Some(mp4)) = (args.first(), args.get(1)) else {
        eprintln!("usage: iskariel-capture decode-proof <in.h264> <out.mp4>");
        return ExitCode::from(2);
    };
