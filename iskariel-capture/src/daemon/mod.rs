...
// Linux, a `\\.\pipe\iskariel-capture` named-pipe arm on Windows (SF4). The generic
...
/// `iskariel-capture daemon` (Windows, Game Capture SF4). The named-pipe control
...
/// (`spawn_capture`) + the event drain, then serves `\\.\pipe\iskariel-capture` over
...
pub fn run(_args: &[String]) -> ExitCode {
    ...
    let runtime = match tokio::runtime::Builder::new_multi_thread().enable_all().build() {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("iskariel-capture daemon: failed to build tokio runtime: {e}");
    ...
    runtime.block_on(async move {
        ...
        match socket::serve(ctx).await {
            ...
            Err(e) => {
                eprintln!("iskariel-capture daemon: control pipe error: {e}");
                ExitCode::FAILURE
            }
        }
    })
...
/// `iskariel-capture daemon` entry. Builds the multi-thread tokio runtime, the
    ...
    runtime.block_on(async move {
        ...
        match socket::serve(ctx).await {
            ...
            Err(e) => {
                ...
                eprintln!("iskariel-capture daemon: socket server error: {e}");
                ExitCode::FAILURE
            }
        }
    })
...
fn materialize_chime() -> Option<PathBuf> {
    let dir = PathBuf::from(std::env::var_os("XDG_RUNTIME_DIR")?).join("iskariel");
