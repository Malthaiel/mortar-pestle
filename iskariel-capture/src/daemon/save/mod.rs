...
/// `ISKARIEL_CAPTURES_DIR` nor `%USERPROFILE%` is set (no per-clip FIFOs on Windows).
#[cfg(windows)]
fn runtime_dir() -> PathBuf {
    std::env::temp_dir().join("iskariel")
}
...
/// The Library captures root: `ISKARIEL_CAPTURES_DIR` (set by the app's engine
...
fn captures_root() -> PathBuf {
    std::env::var_os("ISKARIEL_CAPTURES_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| runtime_dir().join("captures"))
}

/// Windows captures root: `ISKARIEL_CAPTURES_DIR` (the app sets it) → default
...
#[cfg(windows)]
fn captures_root() -> PathBuf {
    if let Some(dir) = std::env::var_os("ISKARIEL_CAPTURES_DIR") {
