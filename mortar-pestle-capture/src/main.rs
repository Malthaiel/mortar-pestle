//! `mortar-pestle-capture` — the Game Capture engine.
//!
...
const USAGE: &str = "\
usage: mortar-pestle-capture <command>
...
fn main() -> ExitCode {
    ...
    match (args.first().map(String::as_str), args.get(1).map(String::as_str)) {
        ...
        (Some("spike"), Some(name @ ("color" | "tear" | "pacing"))) => {
            eprintln!("mortar-pestle-capture: spike '{name}' not yet implemented (Stage B)");
            ExitCode::from(2)
        }
        (Some("spike"), other) => {
            eprintln!("mortar-pestle-capture: unknown spike {other:?}");
            ExitCode::from(2)
        }
        #[cfg(target_os = "linux")]
...
fn capture_record(args: &[String]) -> ExitCode {
    ...
    let Some(out) = args.first() else {
        eprintln!("usage: mortar-pestle-capture capture record <out.mp4|dir> [secs]");
        return ExitCode::from(2);
    };
    let secs: u64 = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(8);
    let dir = gate_dir(out);
    let _ = std::fs::create_dir_all(&dir);
    std::env::set_var("MORTAR_PESTLE_CAPTURES_DIR", &dir);
...
fn capture_replay(args: &[String]) -> ExitCode {
    ...
    env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("info")).init();
    let Some(out) = args.first() else {
        eprintln!("usage: mortar-pestle-capture capture replay <dir> [secs]");
        return ExitCode::from(2);
    };
    let secs: u64 = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(8);
    let dir = gate_dir(out);
...
/// SF4 gate (Windows): connect to the daemon's control pipe (`\\.\pipe\mortar-pestle-capture`)
...
/// Launch the daemon first (`mortar-pestle-capture daemon`), then run this in a 2nd process;
...
fn capture_probe_pipe(_args: &[String]) -> ExitCode {
    ...
    const PIPE_NAME: &str = r"\\.\pipe\mortar-pestle-capture";
            ...
            Ok(Err(e)) => {
                eprintln!(
                    "\nVERDICT: SF4 FAILED — could not open {PIPE_NAME}: {e}\n  \
                     (is the daemon running? `mortar-pestle-capture daemon`)"
                );
                return ExitCode::from(1);
            }
            Err(_) => {
